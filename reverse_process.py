...
Borrow from the code described below, modified by Yu Feng@YuFengUofR
...
def random_ccm():
  ...
  xyz2cams = np.array([[[1.0234, -0.2969, -0.2266],
                ...
                [[0.6596, -0.2079, -0.0562],
                 ...
                 [-0.097, 0.1581, 0.5181]]], dtype=np.float32)
...
def unprocess(image):
  ...
  with tf.name_scope('unprocess'):
    ...
    # print("image after tone mapping", image.shape, tf.reduce_max(image), tf.reduce_min(image))
    # Inverts gamma compression.
    image = gamma_expansion(image)
    # print("image after gamma_expansion", tf.reduce_max(image), tf.reduce_min(image))
    # Inverts color correction.
    image = apply_ccm(image, rgb2cam)
    # print("image after color correction", tf.reduce_max(image), tf.reduce_min(image))
    # # Approximately inverts white balance and brightening.
    image = safe_invert_gains(image, rgb_gain, red_gain, blue_gain)
    # Clips saturated pixels.
    image = tf.clip_by_value(image, 0.0, 1.0)
    # print("image after reverse gain", tf.reduce_max(image), tf.reduce_min(image))
    # Applies a Bayer mosaic.
    image = mosaic(image)
    # print("image after mosaic", tf.reduce_max(image), tf.reduce_min(image))
